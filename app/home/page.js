    ...
    <div className="w-full h-screen flex flex-col md:flex-row relative overflow-hidden bg-(--bg-main)">
      ...
      {isMobile ? (
        <div className="w-full h-full relative">
          ...
          <div className={`absolute top-0 left-0 h-full w-[80%] max-w-[320px] z-50 transition-transform duration-300 ${isSideBarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            <Sidebar state={isSideBarOpen} func={toggleSideBar} func2={toggleCreateProject} />
          </div>

          {/* Mobile Conversation Overlay */}
          <div className={`absolute top-0 right-0 h-full w-[85%] max-w-[350px] z-50 transition-transform duration-300 ${isConvoOpen ? 'translate-x-0' : 'translate-x-full'}`}>
            <Conversation onClose={toggleConvo} />
          </div>
        </div>
      ) : (
        <Group
          className="h-screen w-full flex gap-0"
          orientation="horizontal"
        >
          <Panel defaultSize={15} minSize={15} maxSize={30} hidden={!isSideBarOpen}>
            <Sidebar
              state={isSideBarOpen}
              func={toggleSideBar}
          ...
          <Separator />
          <Panel defaultSize={25} minSize={20} maxSize={40} hidden={!isConvoOpen}>
            <Conversation onClose={toggleConvo} />
          </Panel>
        </Group>
      )}
