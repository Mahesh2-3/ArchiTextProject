...
router.put("/password", async (req, res) => {
  ...
  const { success, message } = await changePassword(req.user._id, oldPassword, newPassword);
  if (!success) {
...
router.post("/reset-password", async (req, res) => {
  ...
  const { success, message } = await resetPasswordWithOtp(req.user._id, otp, newPassword);
  if (!success) {
    return res.status(400).json({ success: false, message });
  }
